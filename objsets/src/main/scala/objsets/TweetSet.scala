...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList =
    if(isEmpty) Nil
      ...
      val mos = mostRetweeted
      new Cons(mos, remove(mos).descendingByRetweet)
    }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet =
    ...
    else if (left.isEmpty) if (elem.retweets > right.mostRetweeted.retweets) elem else right.mostRetweeted
    else if (right.isEmpty) if (elem.retweets > left.mostRetweeted.retweets) elem else left.mostRetweeted
    else if (right.mostRetweeted.retweets > left.mostRetweeted.retweets) right.mostRetweeted
    else left.mostRetweeted

  def union(that: TweetSet): TweetSet =
    that.filterAcc(tw => true,
      left.filterAcc(tw => !that.contains(tw),
        right.filterAcc(tw => !that.contains(tw),
